...
if (!empty($patient_name)) {
    ...
    $sql_patient = "SELECT patient_id FROM tbl_inpatient_record WHERE patient_name = ?";
    ...
    if ($stmt_patient = mysqli_prepare($connection, $sql_patient)) {
        ...
        if ($row_patient = mysqli_fetch_assoc($result_patient)) {
            ...
            $sql_lab_tests = "SELECT lab_test, price FROM tbl_laborder WHERE patient_id = ?";
            if ($stmt_lab_tests = mysqli_prepare($connection, $sql_lab_tests)) {
                ...
                if (mysqli_num_rows($result_lab_tests) > 0) {
                    ...
                    echo json_encode([]); // No lab tests found for this patient
                }
